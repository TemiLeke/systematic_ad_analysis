...
def isolated_labels(
    ...
    for label in isolated_labels:
        score = score_isolated_label(
            adata, label_key, cluster_key, label, embed, recluster, get_max_f1, resolutions, verbose=verbose
        )
        scores[label] = score
...
def score_isolated_label(
    ...
    elif not recluster and get_max_f1:
        iso_label_key = cluster_key
        score = max_f1(
            adata_tmp, label_key, iso_label_key, isolated_label, argmax=False
        )        
    elif not recluster and not get_max_f1:
