...
.myMapToHuman_org=function(inputData,server=F){
  
  {
    ...
    if(!file.exists("~/serverFiles/mouse_human_orthologs.rda")){
      stop("Error! file ~/serverFiles/mouse_human_orthologs.rda is missing")
    }
  ...
  
  {
    ...
    if(conservativeMapping|oldMapping){
      if(!file.exists("~/serverFiles/ortholog_mapping_old.rda")){
        stop("Error! file ~/serverFiles/ortholog_mapping_old.rda is missing")
      }
      load("~/serverFiles/ortholog_mapping.rda")
      mapping_old=mapping
    } else if(MGIsymbol){
      if(!file.exists("~/serverFiles/mouse_human_orthologs_MGISymbol_03062021.rda")){
        stop("Error! file ~/serverFiles/mouse_human_orthologs_MGISymbol_03062021.rda is missing")
      }
...
.myAnnotateFn=function(marker_data,marker_count_thr=100,pct.2_thr=0.5,ncores=10,gene_count_per_cluster=500){
  ...
  if(!file.exists("~/serverFiles/enrichr_azimuth_2021.gmt")){
    stop("Error! file ~/serverFiles/enrichr_azimuth_2021.gmt is missing")
  }
  
  ctype_markers=.sconline.GSEA.readGMT(file="~/serverFiles/enrichr_azimuth_2021.gmt",bkg_genes=NULL,min.gs.size=NULL,max.gs.size=NULL)
